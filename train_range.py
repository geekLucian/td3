...
def train(exp_name='multi_agent',  # 指定本次实验的命名
    ...
    # 重置环境
    obs_n, combine_reward0, clear_price = env.reset()
    ...
    while True:
        ...
        if logger.episode_count > 180:
            action_n = [agent.action(obs.astype(np.float32), 0.2 / (logger.train_step / 1500 + 1)) for agent, obs in zip(agents, obs_n)]
        else:
            action_n = [agent.policy.sample_action().numpy() for agent, obs in zip(agents, obs_n)]
            # print(action_n)

        # 执行动作
        new_obs_n, profit, clear_price, total_match_volume, seller_clear_volume, buyer_clear_volume, match_result, seller_profit, done, _ = env.step(action_n)
        ...
        if done:
            if logger.episode_count % 100 == 0:
                print('最终出清价格-每一幕', clear_price)
                print("最终匹配对-每一幕", match_result)
            ...
            obs_n, combine_reward0, clear_price = env.reset()  # 重置场景，拿到新的状态
